...
bool
Value::wantCString(char * buffer, unsigned int size) const
{
...
Value::wantString() const
{
   ...
   case intValue:
	   {
		   if (sizeof(value_.int_) == sizeof(long long))
		   {
#ifdef WIN32
			   sprintf(buffer, "%I64d", value_.int_);
#else
			   sprintf(buffer, "%lld", value_.int_);
#endif
		   }
		   else if(sizeof(value_.int_) == sizeof(int))
		   {
			   sprintf(buffer, "%d", value_.int_);
		   }
		   else
		   {
			   buffer[0] = 0;
   ...
   case uintValue:
	   {
		   if (sizeof(value_.uint_) == sizeof(unsigned long long))
		   {
#ifdef WIN32
			   _ui64toa(value_.uint_, buffer, 10);
#else
			   sprintf(buffer, "%llu", value_.uint_);
#endif
		   }
		   else if(sizeof(value_.uint_) == sizeof(unsigned int))
		   {
			   sprintf(buffer, "%u", value_.uint_);
		   }
		   else
		   {
			   buffer[0] = 0;
   ...
   case realValue:
	   {
		   sprintf(buffer, "%.05f", value_.real_);
	   }
	   return buffer;
   case nullValue:
   case arrayValue:
...
Value::wantInt() const
{
   ...
   case stringValue:
	   {
		   if (NULL == value_.string_) {
			   return 0;
		   }
		   return Int(atoi(value_.string_));
	   }
	   break;
   case arrayValue:
   case objectValue:
   ...
   case stringValue:
	   {
		   ...
		   return UInt(atoi(value_.string_));
	   }
	   break;
   case arrayValue:
   case objectValue:
   ...
   case stringValue:
	   {
		   ...
		   return Int64(_atoi64(value_.string_));
	   }
	   break;
   case arrayValue:
   case objectValue:
   ...
   case stringValue:
	   {
		   ...
		   return UInt64(_atoi64(value_.string_));
	   }
	   break;
   case arrayValue:
   case objectValue:
   ...
   case stringValue:
	   {
		   ...
		   return float(atof(value_.string_));
	   }
	   break;
   case arrayValue:
   case objectValue:
...
const Value& Value::resolve(const std::string& path) const
{
	return Path(path).resolve(*this);
}
